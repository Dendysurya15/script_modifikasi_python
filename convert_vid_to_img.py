...
def save_img_async(img, random_code):
    dt = date.today()
    ...
    directory_path = './hasil_split/' + date_folder +'/' + str(file_name_without_extension) +'/'
    ...
    img_path = directory_path + f"{random_code}.JPG"
...
def capture_screenshots():
    while not screenshot_thread_stop_flag.is_set():
        screenshot_event.wait()  # Wait for the event to be set
        screenshot_event.clear()  # Clear the event for the next iteration
        random_code = generate_random_code()
        index = 100
        dt = date.today()
        for _ in range(3):
            _, frame = cap.read()
            save_img_async(frame,str(dt) + '_' + random_code + '_' + str(index)+'ms')
            index += 100
            sleep(0.1)
